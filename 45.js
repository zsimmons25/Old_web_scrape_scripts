...
var links = document.querySelectorAll('#ctl00_ContentPlaceHolderMiddleColumn_GridView1>tbody>tr>td>a');
...
var secondaries = document.querySelectorAll('#ctl00_ContentPlaceHolderMiddleColumn_dlDetails1>tbody>tr:nth-of-type(4)>td>table>tbody>tr>td:nth-of-type(1)');
for (x = 0; x < secondaries.length; x++) {
	secondaries[x].textContent = secondaries[x].textContent + '_x'
}
...
new_table_.setAttribute('class', 'redacted');
...
if (rows[0] != undefined) {
    for (i of rows) {
        var lic_ = document.createElement('tr');
        lic_.setAttribute('class', 'redacted');
    ...
    blank_row.setAttribute('class', 'redacted');
    for (i of header_a) {
