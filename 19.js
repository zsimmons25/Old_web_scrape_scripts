...
let header_a = ['Name','First Name','Last Name','License Type','License Number','License Status','License Issue Date','License Expiration Date','Obtained By','License Action'];
...
if (rows[1] != undefined) {
	for (let i = 1; i < rows.length; i++) {
		...
		let type_ = values_[1].textContent;
		let number_ = values_[2].textContent;
		let status_ = values_[3].textContent;
		let lid_ = values_[4].textContent;
		let obtained_ = values_[5].textContent;
		let led_ = values_[6].textContent;
		let disc_ = values_[7].textContent;
		[name_,first_name,last_name,type_,number_,status_,lid_,led_,obtained_,disc_].forEach(x => lic_.insertCell(-1).textContent = x);
